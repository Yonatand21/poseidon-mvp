#!/usr/bin/env python3
"""Minimal AUV runtime publisher for federated MVP bring-up."""
...
class MockAuvRuntime(Node):
    def __init__(self) -> None:
        super().__init__("mock_auv_runtime")
        self.declare_parameter("seed", 42)
        self.declare_parameter("radius_m", 80.0)
        self.declare_parameter("speed_mps", 1.5)
        self.declare_parameter("depth_m", 25.0)
        ...
        self._depth = float(self.get_parameter("depth_m").value)
        self._omega = self._speed / max(self._radius, 1e-3)
        self._phase = (seed % 360) * math.pi / 180.0
        self._start_ns = self.get_clock().now().nanoseconds

        self._state_pub = self.create_publisher(Odometry, "/auv/state", QOS)
