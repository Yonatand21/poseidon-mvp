...
| **T1** | Mac M4 with Docker Desktop | ~10 min | Build poseidon-base-dev for the host arch, smoke-test ros2/python/uv, colcon-build mock package, compose up federated core profile, verify `/auv/state`, `/ssv/state`, `/scenario/clock`. |
| **T2** | Mac M4 | ~3 min | Streamlit dashboard launches and reads recordings dir. |
| **T3** | Linux + NVIDIA GPU (cloud box) | ~60 min | setup-linux.sh end-to-end, cloud-demo-box.provision.sh, full compose with GPU overlay, federated runtime topics and visual profile healthy. |
...
- Missing ROS 2 desktop dependencies in the base image. If one fails,
  update `deploy/docker/base-dev.Dockerfile` package list for Ubuntu 24.04.
...
sleep 20
docker compose logs sim-auv | tail -30
docker compose logs sim-ssv | tail -30
docker compose logs federation-bridge | tail -30
docker run --rm --network container:poseidon-mvp-sim-auv-1 \
    poseidon-base-dev:latest ros2 topic list | egrep '^/auv/state$|^/ssv/state$|^/scenario/clock$'
```

Pass = `/auv/state`, `/ssv/state`, and `/scenario/clock` are present and
federation bridge logs show synchronized state updates.
...
| T0 + T1 | base-dev.Dockerfile, mock runtime package, docker-compose.yml, docker-compose.gpu.yml |
| T0 + T1 + T3 | GPU overlay and cloud demo profile |
